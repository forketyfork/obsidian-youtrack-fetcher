...
- You can specify arbitrarily nested fields, e.g., `${reporter.manager.fullName}`
...
The plugin creates notes with the following format. As an alternative, specify your own template in the plugin settings. You can use arbitrarily nested fields, e.g., `${reporter.manager.fullName}`.
...
Any field referenced in the template can be used as a placeholder with `${field}`. You can also use arbitrarily nested fields, e.g., `${reporter.manager.fullName}`.
